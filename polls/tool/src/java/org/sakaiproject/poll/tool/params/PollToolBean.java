...
public class PollToolBean {
	...
	private PollVoteManager pollVoteManager;
        ...
        public String pollCategoryName, pollCategoryId;
        
        public PollCategory pollCategory;
        
        private OtherOption otherOption;
        public String otherOptionText, otherOptionVoter, otherOptionPollId;
        
        public String academicTermTitle, academicTermDescription, academicTermCreator;
        public String academicTermStartDateString, academicTermEndDateString;
        public Date academicTermStartDate, academicTermEndDate;
	...
	private void sendOptionDeletedNotification(String[] userEids) {
        ...
        public String processActionAddGlossary(){
            if (glossaryCategory != null && glossaryName != null && glossaryDescription != null) {
        ...
        public String processActionAddPollCategory(){
            if (pollCategoryName != null) {
                PollCategory newPollCategory = new PollCategory();
                boolean status;
                
                newPollCategory.setName(pollCategoryName);
                status = manager.savePollCategory(newPollCategory);
                if(status){
                    return "success";
                }
        ...
        public String processActionUpdatePollCategory(){            
            if (pollCategoryName != null && pollCategoryId != null) {
                PollCategory editedPollCategory = manager.getPollCategory(Long.valueOf(pollCategoryId));
                boolean status;
                
                editedPollCategory.setName(pollCategoryName);
                status = manager.updatePollCategory(editedPollCategory);
                if(status){
                    return "success";
                }
...
//        public String processActionAddOtherOption(){
//            otherOption.setOptionText(otherOption.getOptionText());
//            
//            boolean status;
//            
...
//        }
        ...
        public String processActionAddOtherOption(){
            if (otherOptionText != null) {
                OtherOption newOtherOption = new OtherOption();
                boolean status;
                
                newOtherOption.setOptionText(otherOptionText);
                newOtherOption.setUserId(otherOptionVoter);
                newOtherOption.setPollId(Long.parseLong(otherOptionPollId));
                status = manager.saveOtherOption(newOtherOption);
                if(status){
                    return "success";
                }
        ...
        public String processActionAddAcademicTerm() throws ParseException{            
            if (academicTermTitle != null && academicTermDescription != null && academicTermStartDateString != null
                    && academicTermEndDateString != null) {
                
                SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-mm-dd");
                
                AcademicSession newAcademicSession = new AcademicSession();
                
                academicTermStartDate = dateFormat.parse(academicTermStartDateString);
                academicTermEndDate = dateFormat.parse(academicTermEndDateString);
                
                if (academicTermStartDate.before(academicTermEndDate)) {
                    boolean status, isCurrent;                
                    int version = 1;                           
                    if (academicTermStartDate.before(new Date()) && academicTermEndDate.after(new Date())) {
                        isCurrent = true;
                    }else{
                        isCurrent = false;
                    }                    
                    newAcademicSession.setTitle(academicTermTitle);
                    newAcademicSession.setEnterpriseId(academicTermTitle);
                    newAcademicSession.setDescription(academicTermDescription);
                    newAcademicSession.setVersion(Long.valueOf(version));                
                    newAcademicSession.setIsCurrent(isCurrent);
                    newAcademicSession.setCreatedDate(new Date());
                    newAcademicSession.setCreatedBy(academicTermCreator);
                    newAcademicSession.setStartDate(academicTermStartDate);
                    newAcademicSession.setEndDate(academicTermEndDate);                
                    status = manager.saveAcademicSession(newAcademicSession);
                    if(status){
                       return "success";
                    }
