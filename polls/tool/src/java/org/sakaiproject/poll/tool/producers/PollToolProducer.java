...
DefaultView,NavigationCaseReporter {
	...
	private static final String NAVIGATE_VOTE = "poll-vote";
        ...
        private static final String NAVIGATE_POLL_CATEGORY = "actions-poll-category";        
        private static final String NAVIGATE_USER_NOT_VOTE = "actions-user-not-vote";
        private static final String NAVIGATE_POLL_BY_CATEGORY = "actions-poll-by-category";
        private static final String NAVIGATE_PRODUCT = "actions-product";
        private static final String NAVIGATE_PRODUCT_TYPE = "actions-product-type";
        private static final String NAVIGATE_CUSTOMER = "actions-customer";
        private static final String NAVIGATE_TRANSACTION = "actions-transaction";
        private static final String NAVIGATE_ADD_TRANSACTION = "actions-add-transaction";
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams,
		...
		if (this.isAllowedPollAdd() || this.isSiteOwner() ) {
			...
			if (this.isSiteOwner()) {
				UIInternalLink.make(actions, NAVIGATE_PERMISSIONS, UIMessage.make("action_set_permissions"),new SimpleViewParameters(PermissionsProducer.VIEW_ID));
                                ...
                                UIInternalLink.make(actions, NAVIGATE_POLL_CATEGORY, UIMessage.make("action_poll_category"),new SimpleViewParameters(PollCategoryProducer.VIEW_ID));                                
                                ...
                                UIInternalLink.make(actions, NAVIGATE_POLL_BY_CATEGORY, UIMessage.make("action_poll_by_category"),new PollViewParameters(PollByCategoryProducer.VIEW_ID, null));
                                UIInternalLink.make(actions, NAVIGATE_PRODUCT, "List Product",new SimpleViewParameters(ProductProducer.VIEW_ID));
                                UIInternalLink.make(actions, NAVIGATE_PRODUCT_TYPE, "List Product Type",new SimpleViewParameters(ProductTypeProducer.VIEW_ID));
                                UIInternalLink.make(actions, NAVIGATE_CUSTOMER, "List Customer",new SimpleViewParameters(CustomerProducer.VIEW_ID));
                                UIInternalLink.make(actions, NAVIGATE_TRANSACTION, "List Transaction",new SimpleViewParameters(TransactionProducer.VIEW_ID));
                                UIInternalLink.make(actions, NAVIGATE_ADD_TRANSACTION, "Add Transaction",new PollViewParameters(AddTransactionProducer.VIEW_ID, null));
		...
		for (int i = 0 ; i < polls.size(); i++) {
			...
			boolean canVote = pollVoteManager.pollIsVotable(poll);
                        boolean userHasVoteOtherOption = pollVoteManager.userHasVoteOtherOption(poll.getPollId());
			UIBranchContainer pollrow = UIBranchContainer.make(deleteForm,
					canVote && !userHasVoteOtherOption ? "poll-row:votable"
							: "poll-row:nonvotable", poll.getPollId().toString());
			log.debug("adding poll row for " + poll.getText());

			if (canVote && !userHasVoteOtherOption) {
			...
			if (pollCanDelete(poll)) {
				...
				renderDelete = true;
                        ...
                        if(poll.getPollCategoryId() != null){
                            ...
                            UIOutput.make(pollrow, "poll-category", pollCategory.getName());                    
                        }else{
                            UIOutput.make(pollrow, "poll-category", messageLocator.getMessage("no_poll_category"));	
                        }
		}
