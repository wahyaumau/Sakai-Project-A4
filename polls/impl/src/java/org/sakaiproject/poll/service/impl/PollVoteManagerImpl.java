...
public class PollVoteManagerImpl implements PollVoteManager {
    ...
    public List<String> getUserIdUsersNotVote(String siteId) {
        return dao.getUserIdUsersNotVote(siteId);
    }

    @Override
    public List<String> getUsernameUsersNotVote(String siteId) {
        return dao.getUsernameUsersNotVote(siteId);
    }

    @Override
    public List<String> getPollTextUsersNotVote(String siteId) {
        return dao.getPollTextUsersNotVote(siteId);
    }

    @Override
    public int getUsersNotVoteCount(String siteId) {
        return dao.getUsersNotVoteCount(siteId);
    }

    @Override
    public int getUsersInPollSiteCount(String siteId, Poll poll) {
        return dao.getUsersInPollSiteCount(siteId, poll);
    }
    
    @Override
    public boolean userHasVoteOtherOption(Long pollid, String userID) {
		Search search = new Search();
		search.addRestriction(new Restriction("userId",userID));
		search.addRestriction(new Restriction("pollId",pollid));
		List<OtherOption> otherOptions = dao.findBySearch(OtherOption.class, search);		                
                return otherOptions.size() > 0;
	}
    
    @Override
    public boolean userHasVoteOtherOption(Long pollId) {
		return userHasVoteOtherOption(pollId, externalLogic.getCurrentUserId());
	}
}
